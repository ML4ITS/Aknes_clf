...
class FinetuneExperiment(pl.LightningModule):
    ...
    def forward(self, st: Tensor) -> Tensor:
        return self.encoder_ts(st)
    ...
    def training_step(self, batch, batch_idx):
        ...
        (st, _, _), label = batch  # st: (B, n_imfs, L)
        st = st.float()
        label = label.long()

        y = self.forward(st)  # (B, D)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        st, label = batch  # st: (B, n_sensors, n_imfs, L)
        st = st.float()
        label = label.long()
        label = label.cpu()

        # stack representations along the sensor-axis
        n_sensors = st.shape[1]
        ys = None
        for sensor_idx in range(n_sensors):
            st_view_single_sensor = st[:, sensor_idx, :, :]  # (B, n_imfs, L)
            y = self.forward(st_view_single_sensor).detach()  # (B, D)

            if ys is None:
                ys = torch.zeros(y.shape[0], n_sensors, y.shape[-1]).float().to(st.device)  # [B x 24 x D]
            ys[:, sensor_idx, :] = y
    ...
    def configure_optimizers(self):
        ...
        opt_params.append({'params': self.encoder_ts.parameters(), 'lr': self.params['LR_enc']['encoder']})
