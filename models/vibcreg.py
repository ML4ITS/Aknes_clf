...
def compute_vibcreg_loss(z1: Tensor, z2: Tensor, params: dict, kind: str, loss_hist: dict):
    ...
    loss_hist[f'{kind}/sim_loss'] = sim_loss
    loss_hist[f'{kind}/var_loss'] = var_loss
    loss_hist[f'{kind}/cov_loss'] = cov_loss
...
class VIbCReg(nn.Module):
    def __init__(self,
                 out_size_enc: int = 512,
                 proj_hid: int = 4096,
                 proj_out: int = 4096,
                 backbone_type: str = 'ResNet34Encoder',
                 **kwargs):
        super().__init__()
        self.out_size_enc = out_size_enc

        self.encoder = backbones[backbone_type]()
        self.projector = Projector(out_size_enc, proj_hid, proj_out)

    def forward(self, Sxx: Tensor) -> (Tensor, Tensor):
        y = self.encoder(Sxx)
        z = self.projector(y)
        return y, z
    ...
    def loss_function(self,
                      ...
                      z_l: Tensor,
                      params: dict = None,
                      loss_hist: dict = None) -> dict:
        vibcreg_loss = compute_vibcreg_loss(z, z_l, params, 'spectrogram', loss_hist)
        return vibcreg_loss
